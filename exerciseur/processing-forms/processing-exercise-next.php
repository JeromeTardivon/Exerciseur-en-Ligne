...
if (!isset($_SESSION["user"])) {
    ...
    if($dbi->haveUserDoneExercise($_SESSION['user']['id'],$_GET['id-chapter'], $_GET['exercise-num'])){
        $statement=$db->prepare("UPDATE users_exercises SET answer = :answers WHERE id_user= :idUser AND id_exercise=:idExercise");
        $statement->execute(['answers'=>$_POST['studentAnswer'], 'idUser'=>$_SESSION['user']['id'],'idExercise'=>$idExercise]);
    }else{
...
header("location: /exercise.php?id-chapter=".$_GET['id-chapter']."&exercise-num=".($_GET['exercise-num']+1));
